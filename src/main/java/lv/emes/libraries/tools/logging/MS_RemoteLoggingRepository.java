...
public class MS_RemoteLoggingRepository extends MS_Repository<MS_LoggingEvent, ZonedDateTime> implements MS_LoggingRepository {
    ...
    protected boolean isInitialized() {
        return true; //TODO check if remote server is available by checking status endpoint
    }

    @Override
    protected void doAdd(ZonedDateTime identifier, MS_LoggingEvent item) {
        if (LoggingEventTypeEnum.UNSPECIFIED.equals(item.getType()))
            return; //do nothing for lines, because it doesn't make sense to store them as items with IDs

        String url = getRemoteServerBasePath(serverProperties) + serverProperties.getEndpointPutEvent();
        MS_HttpRequestResult httpResult = MS_HttpClient.put(url, MS_CodingUtils.newSingletonMap("event", JsonWriter.objectToJson(item)));
    ...
    protected void doRemoveAll() {
        String url = getRemoteServerBasePath(serverProperties) + serverProperties.getEndpointDeleteAllEvents();
        MS_HttpRequestResult httpResult = MS_HttpClient.delete(url, null);
    ...
    private String getRemoteServerBasePath(LoggingRemoteServerProperties props) {
        return props.getHost() + ":" + props.getPort() + "/" + props.getEndpointRootName() + "/"
                + this.getProductOwner() + "/" + this.getProductName() + "/";
    }
